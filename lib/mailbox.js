...
Mailbox.prototype.listChildren = function(path, all, callback){
    ...
    if( this.client._capabilities.indexOf("XLIST")>=0){
        command = "XLIST";
    }else if(this.client._capabilities.indexOf("SPECIAL-USE")>=0){
        command = "LIST";
    ...
    this.client._send(command+" "+this.client._escapeString(this.client._rootPath) + " " + path + suffix,
        (function(){
            //console.log( this.client._mailboxList );
            ...
            async.each(
                ...
                function( box, callback ){
                    ...
                    if( that.client._capabilities.indexOf("GETMETADATA") >= 0 ){
                        that.getMetadata( box, '/private/specialuse', callback );
                    }else{
                        callback();
                    }
                ...
                function( error ){
                    ...
                    callback( error, that.client._mailboxList );
                }
...
Mailbox.prototype.subscribeChild = function(path, callback){
    ...
    this.client._send("SUBSCRIBE "+this.client._escapeString(path), (function(status){
        if(typeof callback == "function"){
            if(status == "OK"){
                ...
                callback(new Error("Subcribing to mailbox failed"));
            }
        }
    }).bind(this));
};

Mailbox.prototype.unsubscribeChild = function(path, callback){

    this.client._send("UNSUBSCRIBE "+this.client._escapeString(path), (function(status){
        if(typeof callback == "function"){
            if(status == "OK"){
                callback(null);
            } else {
                callback(new Error("Unsubcribing to mailbox failed"));
            }
        }
    }).bind(this));
};
...
/**
 * Renames an existing mailbox and subscribes to it
 *
 * @param {String} oldName Old name of the mailbox
 * @param {String} newName New name for the mailbox
 * @param {Function} callback Callback function to run with the created mailbox object
 */
Mailbox.prototype.renameChild = function(oldName, newName, callback){

    var oldPath = (this.path ? this.path + this.delimiter + oldName:oldName);
    var newPath = (this.path ? this.path + this.delimiter + newName:newName);

    this.client._send("RENAME " + this.client._escapeString(oldPath) + " " + this.client._escapeString(newPath), (function(status){
        if(status == "OK"){
            this.name = newName;
            if(typeof callback == "function"){
                callback(null, this);
            ...
            callback(new Error("Renaming mailbox failed"));
        }
    }).bind(this));
};
...
Mailbox.prototype.getMetadata = function( box, name, callback  ){
    
    this.client._send("GETMETADATA "+this.client._escapeString(box.path)+" "+name, (function(status){

        if(status == "OK"){
            callback(null, status);
        }else{
            // To Do -> boxes with disabled attribute should be ignored. The other should throw an error
            callback(null, status);
        }
    }).bind(this));
