...
IMAPClient.prototype._onData = function(chunk){
    ...
    if(this._currentMode == this.modes.COMMAND){
        ...
        if(
            data.indexOf('* METADATA') === 0 &&
            (match = data.match(/\{\d+\}\r?\n/) )
        ){
            data = data.substr(0, match.index) + ( data.substr(match.index + match[0].length) || "" );
        }
    ...
    if(data[0]=="*"){
        switch(data[1]){
            ...
            case "OK":
                ...
                return;
            case "METADATA":
                this._handlerMetadata(data.slice(2));
                return;
        }
 ...
 */
IMAPClient.prototype._handlerMetadata = function(list){

    var type     = list[ 1 ][ 0 ] === '/private/specialuse' ? 'type' : 'metadata';
    var metadata = list[ 1 ].slice( 1 ).filter( function( item ){ return item; });

    if( !metadata.length ){
        return;
    }

    for( var i = 0; i < this._mailboxList.length; i++ ){

        if( list[ 0 ] === this._mailboxList[ i ].path ){

            if( type === 'type' ){

                if( metadata.indexOf("\\Inbox")>=0){
                    this._mailboxList[ i ].type = "Inbox";
                }else if( metadata.indexOf("\\All")>=0 ||  metadata.indexOf("\\AllMail")>=0){
                    this._mailboxList[ i ].type = "All Mail";
                }else if( metadata.indexOf("\\Archive")>=0){
                    this._mailboxList[ i ].type = "Archive";
                }else if( metadata.indexOf("\\Drafts")>=0){
                    this._mailboxList[ i ].type = "Drafts";
                }else if( metadata.indexOf("\\Sent")>=0){
                    this._mailboxList[ i ].type = "Sent";
                }else if( metadata.indexOf("\\Junk")>=0 ||  metadata.indexOf("\\Spam")>=0){
                    this._mailboxList[ i ].type = "Junk";
                }else if( metadata.indexOf("\\Flagged")>=0 ||  metadata.indexOf("\\Starred")>=0){
                    this._mailboxList[ i ].type = "Flagged";
                }else if( metadata.indexOf("\\Important")>=0){
                    this._mailboxList[ i ].type = "Important";
                }else if( metadata.indexOf("\\Trash")>=0){
                    this._mailboxList[ i ].type = "Trash";
                }

            }else{
                this._mailboxList[ i ][ type ] = metadata;
            }
...
IMAPClient.prototype.storeMessage = function(message, flags, date, callback){
    ...
    cmd += 'APPEND ';
    cmd += this._escapeString(this._selectedMailbox.path);
    cmd += flags.length ? ' (' + flags.join(' ') + ')' : '';
    ...
    if( date && date instanceof Date && !isNaN( date.getTime() ) ){
        ...
        cmd += date.getDate();
        cmd += '-';
        cmd += MONTHS[date.getMonth()];
        cmd += '-';
        cmd += date.getFullYear();
        cmd += ' ';
        ...
        cmd += ('0' + (-date.getTimezoneOffset() / 60)).slice(-2);
        cmd += ('0' + (-date.getTimezoneOffset() % 60)).slice(-2);
        cmd += '"';
    ...
    this._send( cmd, (function(status, data){
        this._literalHandler = null;
        if(status == "OK"){
        ...
        this._literalHandler = (function(){
            ...
            for( var i = 0 ; i < message.length; i += 4096 ){
                msg.push( message.slice( i, i + 4096 ) );
            }

            msg.forEach( function( chunk, i ){

                setImmediate( function(){
                    that._connection.write( chunk );
                });
 ...
 * Rename a mailbox
 */
IMAPClient.prototype.renameMailbox = function(oldPath, newPath, callback){
    this._rootMailbox.renameChild(oldPath, newPath, callback);
};
...
IMAPClient.prototype.sort= function(query, order, isUid, callback){

    if(!callback && typeof order == "function"){
        callback = order;
        order = undefined;
    }else if(!callback && typeof isUid == "function"){
        callback = isUid;
        isUid = undefined;
    }

    var queryType = isUid ? "UID SORT" : "SORT";
    var self = this;
    ...
    this._send(
      queryType + " (" + order + ") UTF-8 " + queryTerm, (function(status){
        this._collectMailList = false;
        ...
        if(status == "OK"){
            ...
            callback(new Error("Error ordering list"));
        }
    }).bind(this));
};

IMAPClient.prototype.subscribeMailbox = function(path, callback){
    this._rootMailbox.subscribeChild(path, callback);
};

IMAPClient.prototype.unsubscribeMailbox = function(path, callback){
    this._rootMailbox.unsubscribeChild(path, callback);
};
